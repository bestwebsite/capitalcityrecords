...
var SnakepitLoadPosts = function ( $ ) {
	'use strict';
	...
	return {
		...
		loadMorePosts : function () {
			...
			$( document ).on( 'click', '.loadmore-button', function( event ) {
				...
				if ( SnakepitParams.isCustomizer ) {
					...
					alert( SnakepitParams.l10n.infiniteScrollDisabledMsg );
					return;
				}
				...
				$button.html( SnakepitParams.l10n.infiniteScrollMsg ); // loading message
		...
		processContent : function ( response, $button ) {
			...
			if ( response ) {
				...
				if ( SnakepitParams.doLoadMorePaginationHashChange ) {
				...
				if ( max < nextPage || undefined === nextPage || isNaN( nextPage ) ) {
					...
					$button.html( SnakepitParams.l10n.infiniteScrollEndMsg );
					...
					$.post( SnakepitParams.ajaxUrl, {

						action: 'snakepit_ajax_get_next_page_link',
						href : $button.attr( 'href' )
						...
						$button.html( SnakepitParams.l10n.loadMoreMsg );
					} );
				}
		...
		scrollToPoint : function ( scrollPoint ) {
			...
			$( 'html, body' ).stop().animate( {
				...
				scrollTop: scrollPoint - SnakepitUi.getToolBarOffset()
		...
		callBack : function ( $container ) {
			...
			if ( 'undefined' !== typeof SnakepitUi ) {
				SnakepitUi.adjustmentClasses();
				SnakepitUi.resizeVideoBackground();
				SnakepitUi.lazyLoad();
				SnakepitUi.fluidVideos( $container, true );
				SnakepitUi.flexSlider();
				SnakepitUi.lightbox();
				SnakepitUi.addItemAnimationDelay();
				SnakepitUi.parallax();
				SnakepitUi.setInternalLinkClass();
				SnakepitUi.muteVimeoBackgrounds();
			...
			if ( 'undefined' !== typeof SnakepitYTVideoBg ) {
				SnakepitYTVideoBg.init( $container );
				SnakepitYTVideoBg.playVideo( $container );
			}

			if ( $( '.masonry-container' ).length || $( '.metro-container' ).length ) {
				SnakepitMasonry.masonry();
				SnakepitMasonry.resizeTimer();
