 ...
 */
function snakepit_get_fonts_css( $fonts_args ) {

	$fonts_args = wp_parse_args( $fonts_args, [
		'body_font_name',
		'body_font_size',
		'menu_font_name',
		'menu_font_weight',
	...
	/* Body */
	$body_selectors = apply_filters( 'snakepit_body_font_family_selectors', snakepit_list_to_array( 'body, blockquote.wvc-testimonial-content, .tp-caption:not(h1):not(h2):not(h3):not(h4):not(h5)' ) );

	$body_selectors = snakepit_array_to_list( $body_selectors );
	...
	$menu_selectors = apply_filters( 'snakepit_menu_selectors', snakepit_list_to_array( '.nav-menu li, .cta-container' ) );

	$menu_selectors = snakepit_array_to_list( $menu_selectors );
	...
	/* Heading */
	$heading_family_selectors = apply_filters( 'snakepit_heading_family_selectors', snakepit_list_to_array( 'h1, h2, h3, h4, h5, h6, .post-title, .entry-title, h2.entry-title > .entry-link, h2.entry-title, .widget-title, .wvc-counter-text, .wvc-countdown-period, .event-date, .logo-text, .wvc-interactive-links, .wvc-interactive-overlays, .heading-font' ) );

	$heading_selectors = apply_filters( 'snakepit_heading_selectors', snakepit_list_to_array( 'h1:not(.wvc-bigtext), h2:not(.wvc-bigtext), h3:not(.wvc-bigtext), h4:not(.wvc-bigtext), h5:not(.wvc-bigtext), .post-title, .entry-title, h2.entry-title > .entry-link, h2.entry-title, .widget-title, .wvc-counter-text, .wvc-countdown-period, .location-title, .logo-text, .wvc-interactive-links, .wvc-interactive-overlays, .heading-font' ) );

	$heading_family_selectors = snakepit_array_to_list( $heading_family_selectors );
	$heading_selectors = snakepit_array_to_list( $heading_selectors );
	...
	return apply_filters( 'snakepit_fonts_css_output', $font_css, $fonts_args );
}
 ...
 * @return array $fonts
 */
function snakepit_get_template_fonts() {
 ...
 * Customizer preview.
 */
function snakepit_fonts_css_template() {

	$fonts = snakepit_get_template_fonts();
	?>
	<script type="text/html" id="tmpl-snakepit-fonts">
		<?php echo snakepit_get_fonts_css( $fonts ); ?>
	</script>
	<?php
}
add_action( 'customize_controls_print_footer_scripts', 'snakepit_fonts_css_template' );
