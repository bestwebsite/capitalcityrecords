<?php
/**
 * Snakepit background functions
 *
 * @author WolfThemes
 * @category Core
 * @package Snakepit/FRontend
 * @version 1.2.7
 */
 ...
 */
function snakepit_background_overlay( $args ) {
	...
	if ( ( 'custom' === $overlay_color || 'auto' === $overlay_color ) && $overlay_custom_color ) {
		...
		$overlay_style .= 'background-color:' . snakepit_sanitize_color( $overlay_custom_color ) . ';';
	...
	return '<div style="' . snakepit_esc_style_attr( $overlay_style ) . '" class="' . snakepit_sanitize_html_classes( $class ) . '"></div><!--.wvc-bg-overlay-->';
}
 ...
 * @param array $args
 */
function snakepit_background_img( $args = array() ) {
	...
	extract(
		wp_parse_args(
			...
			array(
				...
				'background_effect'       => '',
				'background_img_size'     => 'snakepit-XL',
	...
	$do_object_fit = ( wp_attachment_is_image( $background_img ) && 'no-repeat' === $background_repeat && 'default' !== $background_size && 'parallax' !== $background_effect && ! snakepit_is_edge() && ! wp_is_mobile() );
	...
	if ( $do_object_fit ) {
		...
		$src                = snakepit_get_url_from_attachment_id( $background_img, $background_img_size );
		...
		$img_dominant_color = snakepit_get_image_dominant_color( $background_img );
		...
		$output .= '<div class="' . snakepit_sanitize_html_classes( $container_class ) . '" style="' . snakepit_esc_style_attr( $container_style ) . '">';
		...
		$output .= '<img
			...
			style="' . snakepit_esc_style_attr( $cover_style ) . '"
		...
		$output .= ' class="' . snakepit_sanitize_html_classes( $cover_class ) . '"
		...
		if ( 'parallax' === $background_effect && wp_attachment_is_image( $background_img ) ) {
			...
			$background_color = snakepit_get_image_dominant_color( $background_img );
		...
		if ( 'parallax' !== $background_effect ) {
			...
			if ( wp_attachment_is_image( $background_img ) ) {
				$background_img_url = snakepit_get_url_from_attachment_id( $background_img, $background_img_size );
				$style             .= 'background-image:url(' . esc_url( $background_img_url ) . ');';
			}
		}

		$output .= '<div ' . $attrs . ' class="' . snakepit_sanitize_html_classes( $container_class ) . '" style="' . esc_attr( $style ) . '"></div>';

	} elseif ( snakepit_is_instagram_post() ) {

		$container_class = 'img-bg';

		$instagram_url = snakepit_get_instagram_image_url();
		$output       .= '<div class="' . snakepit_sanitize_html_classes( $container_class ) . '" style="background-size:cover;background-image:url(' . esc_url( $instagram_url ) . ');"></div>';
		...
		$bg_fallback_url = snakepit_get_placeholder_url();

		$output .= '<div class="' . snakepit_sanitize_html_classes( $container_class ) . '" style="background-size:cover;background-image:url(' . esc_url( $bg_fallback_url ) . ');"></div>';
	}
 ...
 * @return string $output
 */
function snakepit_background_slideshow( $args = array() ) {

	extract(
		wp_parse_args(
			$args,
			array(
				'slideshow_image_size' => 'snakepit-XL',
				'slideshow_img_ids'    => '',
	...
	$do_object_fit = ( ! snakepit_is_edge() && ! wp_is_mobile() );
	...
	if ( array() !== $slideshow_img_ids && is_array( $slideshow_img_ids ) ) {
		...
		foreach ( $slideshow_img_ids as $image_id ) {
			...
			$src = esc_url( snakepit_get_url_from_attachment_id( $image_id, $slideshow_image_size ) );

			$output .= '<li class="slide">';

			if ( $do_object_fit ) {

				$output .= wp_get_attachment_image( $image_id, $slideshow_image_size, false, array( 'class' => 'cover' ) );
 ...
 */
function snakepit_background_video( $args = array(), $vimeo = true ) {

	$args = wp_parse_args(
		$args,
		array(
			'video_bg_url'        => snakepit_get_first_video_url(),
	...
	if ( 'selfhosted' === snakepit_get_video_url_type( $args['video_bg_url'] ) ) {

		$output .= snakepit_video_bg( $args );

	} elseif ( 'youtube' === snakepit_get_video_url_type( $args['video_bg_url'] ) ) {

		$output .= snakepit_youtube_video_bg( $args );

	} elseif ( 'vimeo' === snakepit_get_video_url_type( $args['video_bg_url'] ) && $vimeo ) {

		$output .= snakepit_vimeo_video_bg( $args );
	}
 ...
 * @return string $output
 */
function snakepit_video_bg( $args ) {
 ...
 * @return string $output
 */
function snakepit_youtube_video_bg( $args ) {
	...
	wp_enqueue_script( 'snakepit-youtube-video-background' );
 ...
 * @return string $output
 */
function snakepit_vimeo_video_bg( $args ) {
	...
	wp_enqueue_script( 'snakepit-vimeo' );
