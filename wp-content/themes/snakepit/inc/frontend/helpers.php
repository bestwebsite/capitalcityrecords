<?php
/**
 * Snakepit frontend utility functions
 *
 * @package WordPress
 * @subpackage Snakepit
 * @version 1.2.7
 ...
 */
function snakepit_get_current_url() {
	global $wp;
 ...
 * @return int
 */
function snakepit_get_last_post_id( $post_type = 'post' ) {
 ...
 */
function snakepit_content_width() {

	$content_width = 1140;

	$GLOBALS['content_width'] = apply_filters( 'snakepit_content_width', $content_width );
}
add_action( 'after_setup_theme', 'snakepit_content_width', 0 );

/**
 * WooCommerce AJAX search result
 */
function snakepit_ajax_search_query( $typed = null ) {
...
if ( ! function_exists( 'snakepit_get_image_dominant_color' ) ) {
	/**
	 * Get dominant color image
	 *
	 * @param int $attachment_id
	 */
	function snakepit_get_image_dominant_color( $attachment_id ) {
 ...
 * @return string
 */
function snakepit_sanitize_style_attr( $string, $compact = true ) {

	if ( $compact ) {
		$string = snakepit_compact_css( $string );
	}
 ...
 * @return string
 */
function snakepit_sample( $text, $num_words = 55, $more = '...' ) {
	$text = wp_strip_all_tags( wp_trim_words( snakepit_clean_post_content( $text ), $num_words, $more ) );
 ...
 */
function snakepit_get_video_url_type( $url ) {
 ...
 * @return string $color
 */
function snakepit_sanitize_color( $color ) {
 ...
 */
function snakepit_sanitize_html_classes( $class, $fallback = null ) {
	if ( is_string( $class ) ) {
		$class = explode( ' ', $class );
	}
 ...
 * @return string
 */
function snakepit_esc_style_attr( $style ) {

	return esc_attr( trim( snakepit_clean_spaces( $style ) ) );
}
 ...
 * @return $string
 */
function snakepit_clean_post_content( $string ) {
 ...
 */
function snakepit_shortcode_preg_match( $shortcode, $content = '' ) {
 ...
 * @return string $html
 */
function snakepit_filter_audio_shortcode_output( $html ) {

	return '<div class="audio-shortcode-container">' . $html . '</div>';
}
add_filter( 'wp_audio_shortcode', 'snakepit_filter_audio_shortcode_output' );
 ...
 * @return string $html
 */
function snakepit_filter_oembed_output( $html, $url, $attr, $post_id ) {
...
add_filter( 'embed_oembed_html', 'snakepit_filter_oembed_output', 10, 4 );
 ...
 * @return string $html
 */
function snakepit_filter_video_shortcode_output( $html ) {

	return '<div class="video-shortcode-container">' . $html . '</div>';
}
add_filter( 'wp_video_shortcode', 'snakepit_filter_video_shortcode_output' );

if ( ! function_exists( 'snakepit_format_number' ) ) {
	/**
	 ...
	 * @return string
	 */
	function snakepit_format_number( $n ) {
 ...
 */
function snakepit_get_color_brightness( $hex ) {
 ...
 * @return string light|dark
 */
function snakepit_get_color_tone( $hex, $index = 210 ) {
 ...
 * @return bool
 */
function snakepit_color_is_black( $hex ) {
 ...
 * @return bool
 */
function snakepit_color_is_white( $hex ) {
 ...
 * @return string
 */
function snakepit_color_brightness( $hex, $percent ) {

	$steps = ( ceil( ( $percent * 200 ) / 100 ) ) * 2;
	$steps = max( -255, min( 255, $steps ) );
	$hex = str_replace( '#', '', snakepit_sanitize_color( $hex ) );
	if ( strlen( $hex ) === 3 ) {
 ...
 */
function snakepit_adjust_brightness( $hexCode, $adjustPercent ) {
 ...
 * @return string $value
 */
function snakepit_sanitize_css_value( $value, $default_unit = 'px', $default_value = '1' ) {
