 ...
 */
function snakepit_add_menu_parent_class( $items ) {
...
add_filter( 'wp_nav_menu_objects', 'snakepit_add_menu_parent_class' );
 ...
 */
function snakepit_nav_menu_social_icons( $item_output, $item, $depth, $args ) {

	$social_icons = snakepit_social_links_icons();
 ...
 */
function snakepit_menu_item_markup( $item_output, $item, $depth, $args ) {
...
add_filter( 'walker_nav_menu_start_el', 'snakepit_menu_item_markup', 10, 4 );
 ...
 */
function snakepit_add_menu_item_link_attributes( $atts, $item, $args ) {
...
add_filter( 'nav_menu_link_attributes', 'snakepit_add_menu_item_link_attributes', 10, 3 );
 ...
 * @return array $classes
 */
function snakepit_add_menu_item_css_classes( $classes, $item, $args ) {
...
add_filter( 'nav_menu_css_class', 'snakepit_add_menu_item_css_classes', 10, 3 );

/**
 * Add a cart menu item in mobile menu
 */
function snakepit_add_cart_menu_item_mobile( $items, $args ) {
	...
	if ( snakepit_display_cart_menu_item() ) {
		if ( $args->theme_location === 'primary' && preg_match( '/mobile/', $args->menu_class ) ) {
			...
			$items .= snakepit_cart_menu_item_mobile( false );
			$items .= '</li>';
		}
	}
...
add_filter( 'wp_nav_menu_items', 'snakepit_add_cart_menu_item_mobile', 10, 2 );
 ...
 * @param string $location
 */
function snakepit_get_menu_args( $location = 'primary', $type = 'desktop', $depth = 3 ) {
	...
	$args = array(
		...
		'depth'          => $depth,
		'fallback_cb'    => 'snakepit_menu_fallback',
 ...
 */
function snakepit_menu_fallback( $args ) {
	...
	$link = $link_before
	. '<a class="no-link-style" href="' . admin_url( 'nav-menus.php' ) . '">' . $before . esc_html__( 'Add Menu', 'snakepit' ) . $after . '</a>'
	. $link_after;
 ...
 */
function snakepit_social_links_icons() {
	...
	return apply_filters( 'snakepit_social_links_icons', $social_links_icons );
}
