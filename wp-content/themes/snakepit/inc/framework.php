 ...
 */
final class Snakepit_Framework {
	...
	/**
	 * Snakepit_Framework Constructor.
	 *
	...
	public function __construct( $options = array() ) {
		...
		do_action( 'snakepit_framework_loaded' );
	}
	...
	public function include_vc_modules() {
		...
		if ( ! snakepit_include_config( 'vc-post-modules.php' ) ) {
			snakepit_include( 'inc/vc-post-modules.php' );
		}
	}
	...
	public function includes() {
				...
				wp_die(
					sprintf(
						wp_kses(
							...
							__( 'Error locating <code>%s</code> for inclusion.', 'snakepit' ),
							array(
								'code' => array(),
							)
						),
		...
		snakepit_include_config( 'config.php' );

		/* Theme custom functions */
		snakepit_include( 'inc/theme-functions.php' );
	...
	public function frontend_includes() {
		...
		foreach ( $frontend_files as $file ) {
			...
			if ( ! locate_template( '/inc/frontend/' . sanitize_file_name( $file ) . '.php', true, true ) ) {
				wp_die(
					/* translators: %s: the file */
					sprintf( snakepit_kses( __( 'Error locating <code>%s</code> for inclusion.', 'snakepit' ) ), esc_attr( $file ) )
				);
			}
		}
	}
	...
	public function admin_includes() {
		...
		foreach ( $admin_config_files as $file ) {
			...
			if ( ! snakepit_include_config( sanitize_file_name( $file ) . '.php', true, true ) ) {
				wp_die(
					sprintf(
						snakepit_kses(
							/* translators: %s: the file */
	...
	public function setup() {
		 ...
		 * to change 'snakepit' to the name of your theme in all the template files
		 */
		load_theme_textdomain( 'snakepit', get_template_directory() . '/languages' );
	...
	public function set_post_thumbnail_sizes() {
		...
		$image_sizes = apply_filters( 'snakepit_image_sizes', $this->options['image_sizes'] );
	...
	public function register_nav_menus() {
		if ( function_exists( 'register_nav_menus' ) ) {
			register_nav_menus( apply_filters( 'snakepit_menus', $this->options['menus'] ) );
		}
	}
} // end class

/**
 * Returns the main instance of SNAKEPIT to prevent the need to use globals.
 *
 * @return Snakepit_Framework
 */
function SNAKEPIT( $options = array() ) { // phpcs:ignore
	return new Snakepit_Framework( $options );
}
