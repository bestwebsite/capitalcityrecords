 ...
 */
function snakepit_file_get_contents( $file ) {

	if ( is_file( $file ) ) {

		$file_uri = snakepit_get_theme_uri( $file );
 ...
 * @return bool
 */
function snakepit_is_wvc_activated() {
 ...
 * @return bool
 */
function snakepit_is_wc_activated() {
 ...
 */
function snakepit_get_default_post_types() {
	return array(
		'post',
		'page',
		'work',
 ...
 * @return array
 */
function snakepit_get_animations() {

	return apply_filters(
		'snakepit_item_animations',
		array(
			'none'            => esc_html__( 'None', 'snakepit' ),
			'fade'            => esc_html__( 'Fade', 'snakepit' ),
			'fade-up'         => esc_html__( 'Fade Up', 'snakepit' ),
			'fade-down'       => esc_html__( 'Fade Down', 'snakepit' ),
			'fade-left'       => esc_html__( 'Fade Left', 'snakepit' ),
			'fade-right'      => esc_html__( 'Fade Right', 'snakepit' ),
			'fade-up-right'   => esc_html__( 'Fade Up Right', 'snakepit' ),
			'fade-up-left'    => esc_html__( 'Fade Up Left', 'snakepit' ),
			'fade-down-right' => esc_html__( 'Fade Down Right', 'snakepit' ),
			'fade-down-left'  => esc_html__( 'Fade Down Left', 'snakepit' ),

			'flip-up'         => esc_html__( 'Flip Up', 'snakepit' ),
			'flip-down'       => esc_html__( 'Flip Down', 'snakepit' ),
			'flip-left'       => esc_html__( 'Flip Left', 'snakepit' ),
			'flip-right'      => esc_html__( 'Flip Right', 'snakepit' ),

			'slide-up'        => esc_html__( 'Slide Up', 'snakepit' ),
			'slide-down'      => esc_html__( 'Slide Down', 'snakepit' ),
			'slide-left'      => esc_html__( 'Slide Left', 'snakepit' ),
			'slide-right'     => esc_html__( 'Slide Right', 'snakepit' ),

			'zoom-in'         => esc_html__( 'Zoom In', 'snakepit' ),
			'zoom-in-up'      => esc_html__( 'Zoom In Up', 'snakepit' ),
			'zoom-in-down'    => esc_html__( 'Zoom In Down', 'snakepit' ),
			'zoom-in-left'    => esc_html__( 'Zoom In Left', 'snakepit' ),
			'zoom-in-right'   => esc_html__( 'Zoom In Right', 'snakepit' ),
			'zoom-out'        => esc_html__( 'Zoom Out', 'snakepit' ),
			'zoom-out-up'     => esc_html__( 'Zoom Out Up', 'snakepit' ),
			'zoom-out-down'   => esc_html__( 'Zoom Out Down', 'snakepit' ),
			'zoom-out-left'   => esc_html__( 'Zoom Out Left', 'snakepit' ),
			'zoom-out-right'  => esc_html__( 'Zoom Out Right', 'snakepit' ),
		)
	);
}
 ...
 * @return array
 */
function snakepit_get_minimum_required_server_vars() {
 ...
 * Get theme root
 */
function snakepit_get_theme_dirname() {
 ...
 * @return string
 */
function snakepit_get_theme_name() {
	$theme = wp_get_theme();
 ...
 * @return string
 */
function snakepit_get_parent_theme_name() {
	$theme = wp_get_theme( snakepit_get_theme_dirname() );
	return $theme->get( 'Name' );
}
 ...
 * @return string
 */
function snakepit_get_theme_version() {
	$theme = wp_get_theme();
 ...
 * @return string
 */
function snakepit_get_parent_theme_version() {
	$theme = wp_get_theme( snakepit_get_theme_dirname() );
	return $theme->get( 'Version' );
}
 ...
 * @return string
 */
function snakepit_get_theme_slug() {

	return apply_filters( 'snakepit_theme_slug', esc_attr( sanitize_title_with_dashes( get_template() ) ) );
}
 ...
 * @return string
 */
function snakepit_get_inherit_mod( $key, $default = '', $post_id = null ) {
	$option = snakepit_get_theme_mod( $key, $default );

	$post_id = ( $post_id ) ? $post_id : snakepit_get_inherit_post_id();
	if ( get_post_meta( $post_id, '_post_' . $key, true ) ) {
		$option = get_post_meta( $post_id, '_post_' . $key, true );
	}

	return apply_filters( 'snakepit_' . $key, $option );
}

if ( ! function_exists( 'snakepit_get_theme_mod' ) ) {
	/**
	 * Get theme mod
	 *
	 * @param  string $key the mod key.
	 ...
	 * @return string
	 */
	function snakepit_get_theme_mod( $key, $default = '' ) {
		...
		if ( isset( $_GET[ $key ] ) && preg_match( '#^[a-zA-Z0-9-_\/]+$#', $_GET[ $key ] ) ) {
			...
			return apply_filters( 'snakepit_mod_' . $key, get_theme_mod( $key, $default ) );
		}
	}
}
 ...
 * @return string
 */
function snakepit_get_option( $index, $key, $default = null ) {
 ...
 */
function snakepit_update_option( $index, $key, $value ) {
 ...
 *
 * Check if the file exists in the child theme with snakepit_locate_file or else check if the file exists in the parent theme
 *
 * @param string $file the file to include.
 */
function snakepit_include( $file ) {
	if ( snakepit_locate_file( $file ) ) {
		return include snakepit_locate_file( $file );
	}
}
 ...
 * Get config dir
 */
function snakepit_get_config_dir() {
 ...
 */
function snakepit_include_config( $file ) {

	return snakepit_include( snakepit_get_config_dir() . $file );
}
 ...
 * @return string
 */
function snakepit_locate_file( $filename ) {
	...
	return apply_filters( 'snakepit_locate_file', $file );
}
 ...
 * @return string
 */
function snakepit_get_theme_uri( $file = null ) {
 ...
 * @return bool
 */
function snakepit_is_external_url( $string ) {
 ...
 * @return string $url
 */
function snakepit_get_url_from_attachment_id( $id, $size = 'thumbnail' ) {
 ...
 * @return string
 */
function snakepit_compact_css( $css, $hard = true ) {
 ...
 * @return string $list
 */
function snakepit_clean_list( $list, $separator = ',' ) {
 ...
 * @return bool
 */
function snakepit_attr_bool( $var ) {
 ...
 * @return string
 */
function snakepit_clean_spaces( $string, $hard = true ) {
 ...
 * @return array
 */
function snakepit_list_to_array( $list, $separator = ',' ) {
	return ( $list ) ? explode( ',', trim( snakepit_clean_spaces( snakepit_clean_list( $list ) ) ) ) : array();
}
 ...
 * @return array
 */
function snakepit_array_to_list( $array, $separator = ',' ) {
	$list = '';
	...
	return snakepit_clean_list( $list );
}
 ...
 */
function snakepit_get_post_attr( $post_id ) {

	$post_attrs = array();

	$post_attrs['id']           = 'post-' . $post_id;
	$post_attrs['class']        = snakepit_array_to_list( get_post_class(), ' ' );
	...
	if ( 'event' === get_post_type() ) {
		$post_attrs['itemscope'] = '';
		$post_attrs['itemtype']  = 'https://schema.org/' . apply_filters( 'snakepit_microdata_event_itemtype', 'MusicEvent' );
	}

	if ( 'product' === get_post_type() ) {
	}

	return apply_filters( 'snakepit_post_attrs', $post_attrs, $post_id );
}
 ...
 * @param int $post_id The post ID.
 */
function snakepit_post_attr( $class = '', $post_id = null ) {

	$post_id = ( $post_id ) ? $post_id : get_the_ID();
	$attrs   = snakepit_get_post_attr( $post_id );
	$output  = '';
 ...
 * @return bool
 */
function snakepit_is_home_as_blog() {
 ...
 * @return bool
 */
function snakepit_is_blog_index() {

	return snakepit_is_home_as_blog() || ( absint( snakepit_get_the_id() ) === absint( get_option( 'page_for_posts' ) ) );
}
 ...
 * @return bool
 */
function snakepit_is_blog() {

	$is_blog = ( snakepit_is_home_as_blog() || snakepit_is_blog_index() || is_search() || is_archive() ) && ! snakepit_is_woocommerce_page() && 'post' == get_post_type();
	return ( true === $is_blog );
}
 ...
 * @return int $id
 */
function snakepit_get_inherit_post_id() {
	...
	$post_id      = null;
	$shop_page_id = ( function_exists( 'snakepit_get_woocommerce_shop_page_id' ) ) ? snakepit_get_woocommerce_shop_page_id() : false;
	...
	if ( ( snakepit_is_blog() || is_search() ) && false === $is_shop_page && false === $is_product_taxonomy ) {
 ...
 */
function snakepit_get_attachement_id_from_title( $title ) {
 ...
 * @return string
 */
function snakepit_add_to_cart( $product_id, $classes = '', $text = '' ) {
	$wc_url = untrailingslashit( snakepit_get_current_url() ) . '/?add-to-cart=' . absint( $product_id );
	...
	return '<a
		...
		class="' . snakepit_sanitize_html_classes( $classes ) . '">' . $text . '</a>';
}

/**
 * gets the current post type in the WordPress Admin
 */
function snakepit_get_admin_current_post_type() {
 ...
 */
function snakepit_get_category_childs_full( $parent_id, $array, $level, &$dropdown ) {
	...
	while ( $i < count( $array ) ) {
		...
		if ( $item->category_parent == $parent_id ) {
			...
			unset( $array[ $key ] );
			$array = snakepit_get_category_childs_full( $item->term_id, $array, $level + 1, $dropdown );
			$keys  = array_keys( $array );
			$i     = 0;
		}
	}
 ...
 */
function snakepit_get_product_cat_dropdown_options() {
	...
	$product_categories_dropdown = array();
	snakepit_get_category_childs_full( 0, $categories, 0, $product_categories_dropdown );
 ...
 * Get product category dropdown options
 */
function snakepit_get_video_cat_dropdown_options() {
	...
	$video_categories_dropdown = array();
	snakepit_get_category_childs_full( 0, $categories, 0, $video_categories_dropdown );
 ...
 */
function snakepit_get_metro_patterns() {
	return array_flip(
		apply_filters(
			'snakepit_metro_pattern_options',
			array(
				'auto'      => esc_html__( 'Auto', 'snakepit' ),
				'pattern-1' => sprintf( esc_html__( 'Pattern %1$d (loop of %2$d)', 'snakepit' ), 1, 6 ),
				'pattern-2' => sprintf( esc_html__( 'Pattern %1$d (loop of %2$d)', 'snakepit' ), 2, 8 ),
				'pattern-3' => sprintf( esc_html__( 'Pattern %1$d (loop of %2$d)', 'snakepit' ), 3, 10 ),
				'pattern-4' => sprintf( esc_html__( 'Pattern %1$d (loop of %2$d)', 'snakepit' ), 4, 8 ),
				'pattern-5' => sprintf( esc_html__( 'Pattern %1$d (loop of %2$d)', 'snakepit' ), 5, 5 ),
				'pattern-6' => sprintf( esc_html__( 'Pattern %1$d (loop of %2$d)', 'snakepit' ), 6, 5 ),
				'pattern-7' => sprintf( esc_html__( 'Pattern %1$d (loop of %2$d)', 'snakepit' ), 7, 6 ),
			)
		)
	);
}
 ...
 * @return string
 */
function snakepit_get_color_scheme_option() {
	return apply_filters( 'snakepit_color_scheme_option', get_theme_mod( 'color_scheme', get_theme_mod( 'skin', 'default' ) ) );
}
