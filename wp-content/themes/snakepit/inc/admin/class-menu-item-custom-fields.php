<?php
/**
 * Snakepit Menu item custom fields example
 *
 ...
 * @since 0.1.0
 */
class Snakepit_Menu_Item_Custom_Fields {
		...
		?>
			<p class="field-_mega-menu description description-wide">
				<label for="edit-_mega-menu-<?php echo esc_attr( $item_id ); ?>">
					...
					<?php esc_html_e( 'Mega Menu (only available for first level items)', 'snakepit' ); ?>
				</label>
			</p>
			...
			<p class="field-_menu-item-not-linked description description-wide">
				<label for="edit-_menu-item-not-linked-<?php echo esc_attr( $item_id ); ?>">
					...
					<?php esc_html_e( 'Mega Menu 2nd level or dropdown item', 'snakepit' ); ?>
				</label>
			</p>
			...
			<p class="field-_menu-item-hidden description description-wide">
				<label for="edit-_menu-item-hidden-<?php echo esc_attr( $item_id ); ?>">
					...
					<?php esc_html_e( 'Hide item on mega menu (for mega menu 2nd level only)', 'snakepit' ); ?>
				</label>
			</p>
			...
			<p class="field-_menu-item-button-style description description-wide">
				<label for="edit-_menu-item-button-style-<?php echo esc_attr( $item_id ); ?>">
					<select name="_menu-item-button-style[<?php echo esc_attr( $item_id ); ?>]">
						<option value=""><?php esc_html_e( 'None', 'snakepit' ); ?></option>
						<option value="primary" <?php selected( get_post_meta( $item_id, '_menu-item-button-style', true ), 'primary' ); ?>><?php esc_html_e( 'Primary', 'snakepit' ); ?></option>
						<option value="secondary" <?php selected( get_post_meta( $item_id, '_menu-item-button-style', true ), 'secondary' ); ?>><?php esc_html_e( 'Secondary', 'snakepit' ); ?></option>
					</select>
					<?php esc_html_e( 'Button Style (only available for first level items)', 'snakepit' ); ?>
				</label>
			</p>
			...
			<p class="field-_menu-item-scroll description description-wide">
				<label for="edit-_menu-item-scroll-<?php echo esc_attr( $item_id ); ?>">
					...
					<?php esc_html_e( 'Scroll to an anchor', 'snakepit' ); ?>
				</label>
			</p>
			<p class="field-_mega-menu-cols description description-wide">
					<label for="edit-_mega-menu-cols-<?php echo esc_attr( $item_id ); ?>"><?php esc_html_e( 'Mega Menu Columns', 'snakepit' ); ?></label>
						<br>
			...
			<p class="field-_mega-menu-tagline description description-wide">
				...
				<?php esc_html_e( 'Optional Mega Menu Tagline (only available for first level items)', 'snakepit' ); ?>
			</p>
			<?php if ( function_exists( 'wvc_get_fontawesome_icons' ) ) : ?>
				<?php
					$wvc_icons = wvc_get_fontawesome_icons();
				?>
				<p class="field-custom description description-wide snakepit-searchable-container">
					<label for="edit-_menu-item-icon-<?php echo esc_attr( $item_id ); ?>"><?php esc_html_e( 'Icon', 'snakepit' ); ?></label><br />
						<span>
						<?php
						printf(
							'<select data-placeholder="%1$s" name="_menu-item-icon[%2$d]" class="snakepit-searchable edit-_menu-item-icon" id="edit-_menu-item-icon-%2$d">',
							esc_html__( 'None', 'snakepit' ),
							$item_id
						);
							  echo '<option value="">' . esc_html__( 'None', 'snakepit' ) . '</option>';
				...
				<p class="field-_menu-item-icon-position description description-wide">
					<label for="edit-_menu-item-icon-position-<?php echo esc_attr( $item_id ); ?>"><?php esc_html_e( 'Icon position', 'snakepit' ); ?></label>
						<br>
						<select name="_menu-item-icon-position[<?php echo esc_attr( $item_id ); ?>]">
							<option value="before" <?php selected( get_post_meta( $item_id, '_menu-item-icon-position', true ), 'before' ); ?>><?php esc_html_e( 'before', 'snakepit' ); ?></option>
							<option value="after" <?php selected( get_post_meta( $item_id, '_menu-item-icon-position', true ), 'after' ); ?>><?php esc_html_e( 'after', 'snakepit' ); ?></option>
						</select>
				</p>
				<?php
			endif;

			echo apply_filters( 'snakepit_menu_meta_fields_markup', ob_get_clean(), $item_id );
	}
...
Snakepit_Menu_Item_Custom_Fields::init();
