 ...
 */
function snakepit_set_wvc_default_google_fonts( $settings ) {

	/* Get theme fonts */
	$theme_google_font_option = snakepit_get_option( 'fonts', 'google_fonts' );
...
add_filter( 'wvc_default_settings', 'snakepit_set_wvc_default_google_fonts' );
 ...
 * @return array $mods
 */
function snakepit_set_default_social_networks( $mods ) {
...
add_filter( 'snakepit_default_mods', 'snakepit_set_default_social_networks' );

/**
 * Define WooCommerce image sizes on theme activation
 *
 * Can be overwritten with the snakepit_woocommerce_thumbnail_sizes filter
 */
function snakepit_woocommerce_image_sizes() {
	...
	$woocommerce_thumbnails = apply_filters(
		'snakepit_woocommerce_thumbnail_sizes',
		array(
			'catalog'   => array(
				'width'  => '400',
				'height' => '400',
...
add_action( 'after_switch_theme', 'snakepit_woocommerce_image_sizes', 1 );

/**
 * Set default WP options on theme activation
 */
function snakepit_default_wp_options_init() {

	if ( ! get_option( snakepit_get_theme_slug() . '_wp_options_init' ) ) {

		/**
		 * A custom hook to set default options on theme activation
		 */
		do_action( 'snakepit_wp_default_options_init' );

		/*
		 * Another custom hook to set default 3rd party plugin options on theme activation
		 */
		do_action( 'snakepit_plugins_default_options_init' );

		/* Default WP options */
		update_option( 'image_default_link_type', 'file' );

		/* Add option to flag that the default mods have been set */
		add_option( snakepit_get_theme_slug() . '_wp_options_init', true );

		update_option( 'wpb_js_gutenberg_disable', true );
	}
}
add_action( 'init', 'snakepit_default_wp_options_init' );
 ...
 * @return array $mods
 */
function snakepit_set_default_customizer_images( $mods ) {

	$default_images = apply_filters( 'snakepit_default_image_mods', array( 'header', 'background' ) );

	foreach ( $default_images as $default_image ) {

		if ( snakepit_get_theme_uri( '/config/' . $default_image . '.jpg' ) ) {
			...
			$movefile = wp_upload_bits( $default_image . '.jpg', null, snakepit_file_get_contents( $filename ) );
			...
			if ( $movefile && isset( $movefile['url'] ) && empty( $movefile['error'] ) ) {
				...
				if ( $attach_id ) {
					$mods[ $default_image . '_image' ] = snakepit_get_theme_uri( '/config/' . $default_image . '.jpg' );
