 ...
 * Set WPBPB as theme
 */
function snakepit_set_as_theme() {
	vc_set_as_theme();
}
add_action( 'vc_before_init', 'snakepit_set_as_theme' );

/**
 * Disable WPBPB frontend
 */
function snakepit_vc_remove_frontend_links() {

	if ( defined( 'WVC_VERSION' ) && version_compare( WVC_VERSION, '3.1.4', '<' ) ) {
		vc_disable_frontend();
	}
}
add_action( 'vc_after_init', 'snakepit_vc_remove_frontend_links' );

/**
 * Add theme accent color to shared colors
 */
function snakepit_wvc_add_accent_color_option( $colors ) {

	$colors = array( esc_html__( 'Theme Accent Color', 'snakepit' ) => 'accent' ) + $colors;

	return $colors;
}
add_filter( 'wvc_shared_colors', 'snakepit_wvc_add_accent_color_option', 14 );

/**
 * Add row css class
 */
function snakepit_wvc_add_row_css_class( $classes ) {

	$classes[] = 'section';

	return $classes;
}
add_filter( 'wvc_row_css_class', 'snakepit_wvc_add_row_css_class' );

/**
 * Set VC default post types
 */
function snakepit_vc_default_post_types() {

	vc_set_default_editor_post_types( snakepit_get_default_post_types() );
}
add_action( 'vc_after_init', 'snakepit_vc_default_post_types' );
 ...
 * @return string
 */
function snakepit_wvc_default_post_types( $post_types ) {
	$default_post_types = snakepit_get_default_post_types();

	return wp_parse_args( $post_types, $default_post_types );
}
add_filter( 'wvc_default_post_types', 'snakepit_wvc_default_post_types' );
 ...
 * @return string
 */
function snakepit_set_wvc_theme_accent_color( $color ) {
	return snakepit_get_inherit_mod( 'accent_color' );
}
add_filter( 'wvc_theme_accent_color', 'snakepit_set_wvc_theme_accent_color' );
 ...
 * @return array
 */
function snakepit_set_wvc_post_types( $post_types ) {

	if ( class_exists( 'Wolf_Portfolio' ) ) {
		$post_types[ esc_html__( 'Work', 'snakepit' ) ] = 'work';
	}

	if ( class_exists( 'Wolf_Albums' ) ) {
		$post_types[ esc_html__( 'Gallery', 'snakepit' ) ] = 'gallery';
	}

	if ( class_exists( 'Wolf_Discography' ) ) {
		$post_types[ esc_html__( 'Release', 'snakepit' ) ] = 'release';
	}

	if ( class_exists( 'Wolf_Videos' ) ) {
		$post_types[ esc_html__( 'Videos', 'snakepit' ) ] = 'videos';
	}

	if ( class_exists( 'Wolf_Events' ) ) {
		$post_types[ esc_html__( 'Events', 'snakepit' ) ] = 'events';
	}

	if ( class_exists( 'WooCommerce' ) ) {
		$post_types[ esc_html__( 'Product', 'snakepit' ) ] = 'product';
	}

	return $post_types;
}
add_filter( 'snakepit_available_post_types', 'snakepit_set_wvc_post_types' );
 ...
 */
function snakepit_add_vc_templates( $default_templates ) {
	...
	$trans_key = '_vc_templates_' . snakepit_get_theme_slug();
